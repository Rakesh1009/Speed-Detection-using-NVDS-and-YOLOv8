...
<p align="center">
  <img src="resources/yolov8.gif" alt="FPS" style="width:100%; max-width:800px;">
</p>
...
You can run the [`download-models.sh`](models/download_models.sh) script to download `onnx` yolov8 models, these files are parsed by [`this parser`](custom_parsers/nvds_customparser_yolov8).
```sh
cd models
...
## Things to consider before you run the application

- There are some assumptions, based off of which the applications accuracy is going to depend.

- In the [`libs.hpp`](src/libs.hpp) file the values of resolutions of the `Input`, `Muxer`, and the `Tiler` was mentioned, change them according to your need.

- The other variables of the setup are the `FOV` values in the `Horizontal` and `Vertical` direction. It is to be noted that they are the half angles that have been used in the code.

- The values of the distance of the camera to the point on the ground that is at the center for the camera has to be calibrated physically.

- The angle of the camera's plane(image plane) with the ground is taken as Alpha_Y and has to be calculated physically.

- Under these assumptions, using basics of `3D cartesian geometry and trigonometry`, this application is accurate.

- Create a file called `inputsources.txt` and paste the path of videos or rtsp url, this application also supports webcam devices which have to be syntaxed as follows.
